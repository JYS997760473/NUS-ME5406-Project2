...
class LogStepsCallback(BaseCallback):
    def __init__(self, log_dir, last_epoch=False, verbose=0):
    ...
    def _on_training_end(self) -> None:
        if self.last_epoch:
            ...
            if os.path.exists(self.log_dir + 'training_data_temp.csv'):
                os.remove(self.log_dir + 'training_data_temp.csv')
            self.results.to_csv(self.log_dir + 'training_data_temp.csv', index=False)
        print("Τraining ends!")


class TqdmCallback(BaseCallback):
    def __init__(self, epoch_timesteps):
        super().__init__()
        self.progress_bar = None
        self.epoch_timesteps = epoch_timesteps

    def _on_training_start(self):
        self.progress_bar = tqdm(total=self.epoch_timesteps)
