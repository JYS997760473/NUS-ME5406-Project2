...
class SAC_class:
    def __init__(self, env, actor_critic=mlp.MLPActorCritic, seed=0, replay_size=int(1e6),
...
def SAC(env, actor_critic=mlp.MLPActorCritic, seed=0, replay_size=int(1e6), gamma=0.99, 
        polyak=0.995, lr=1e-3, alpha=0.2, batch_size=100, num_episode=500):
    """
    ...
    for episode in range(num_episode):
        ...
        while not done:
            ...
            total_steps += 1
            # Store experience to replay buffer
            replay_buffer.store(obs=obs, act=action, rew=reward, next_obs=obs2, done=done)
            # replay_buffer.store(obs, action, reward, obs2, done)
            obs = obs2
            # update
            if total_steps % 50 == 0 and total_steps > 1000:
